...
from exceptions.exceptions import ConfigException
import yaml

CONFIG_PATH = Path("~/.config/cross-critical-backup/config.yaml").expanduser()

@dataclass
class QuickManifestConfig(BaseModel):
...
@dataclass
class RsyncMirroringJob(BaseModel):
    upstream_statepath : Path

@dataclass
class ResticBackJob(BaseModel):
    password_file_path : Path
    target : Path

@dataclass
class CCBConfig(BaseModel):
    repo_dir : Path
    manifest_exclusions : list[str]
    local_zone: str
    ...
    restic_back_job : ResticBackJob

def load_config() -> CCBConfig:
    ...
    try:
        ...
        ccbConfig = CCBConfig(
                    ...
                    encrypted_dir= Path(config["CRITICAL_FILES_ENCRYPTED_DIR"]),
                    ...
                    restic_back_job = ResticBackJob(
                        target=Path(config["RESTIC_JOB"]["target"]), 
                        password_file_path=Path(config["RESTIC_JOB"]["password_file_path"])
                    )
                )
        return ccbConfig
    except KeyError as e:
        raise ConfigException(f"Missing required config key. Config path: {CONFIG_PATH}.") from e
    except ValidationError as e:
        raise ConfigException(f"Config validation error") from e